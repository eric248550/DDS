...
/*
 * @(#)commend_readerService.h    generated by: makeheader    Thu Feb 28 22:12:21 2019
 *
 ...
 * authentication).
 */
#define COMMEND_STATELESS_READER_LOSS_PERCENTAGE "dds.participant.stateless_data_reader.loss_percentage" /* Format: <R|D>,<percentage from 0-100> */

/*
 * lossDuration is the time in seconds after which the samples will not be lost. Min value = 0, max value = 255.
 * lastLostSeqNum is the highest seq number which will be considered to be dropped. If it's value is 0xffff, this parameter will be ignored.
 ...
 *     lossDuration = 0xff;
 * }
 * percentage is the percentage of samples which will be dropped
 * In the case of deterministic loss, (seqnum % 100/percentage) will be dropped. In case of random,
 * random samples will be dropped which will account to overall "percentage" samples lost.
 * Eg Usage: "D,100,-1,-1" drop all the samples for 255 seconds
 *           "R,20,-1,5" drop random samples (overall 20%) but not after 5 seconds
 *           "D,50,100,-1" drop every other sample until their seqNum is less than 100 and until 255 seconds are elapsed.
 */
#define COMMEND_READER_LOSS_PERCENTAGE "dds.data_reader.loss_percentage" /* Format: <Random|Deterministic>,<percentage from 0-100>, lastLostSeqNum, lossDuration */
