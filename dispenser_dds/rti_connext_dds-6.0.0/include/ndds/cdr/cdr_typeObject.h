...
/*
 * @(#)cdr_typeObject.h    generated by: makeheader    Thu Feb 28 22:11:49 2019
 *
...
extern RTICdrDllExport
void *RTICdrTypeObjectFactory_getSerializationBuffer(
        struct RTICdrTypeObjectFactory *self,
        int serializedSize);

extern RTICdrDllExport
void RTICdrTypeObjectFactory_returnSerializationBuffer(
    struct RTICdrTypeObjectFactory *self,
    void *buffer,
    int bufferSize);
...
struct RTICdrTypeObjectAssignabilityProperty {
    ...
    /*e
     * When set to true, the assignability algorithm won't enforce the rule
     * that says that any enum literals that have the same value must also have
     * the same name.
     *
     * For example, A and B would be assignable only if ignoreEnumLiteralNames
     * is set to true:
     *
     * enum Color {
     *    ROJO = 1
     ...
     * };
     */
    RTIBool ignoreEnumLiteralNames;
    ...
    /*e
     * When set to true, the assignability algorithm won't enforce the rule
     * that disallows assigning a string to another string with a smaller
     * bound.
     *
     * For example, string<100> is assignable to string<200> but
     * string<200> is only assignable to string<100> if
     * ignoreStringBounds is true.
     */
    RTIBool ignoreStringBounds;

    /*e
     * When set to true, the assignability algorithm will enforce the rule
     * that disallows assigning a wider type to a narrower type. A type T2 is
     * said to widen type T1 when type T2 contains non-optional fields that are
     * not present in T1.
     */
    RTIBool preventTypeWidening;

    /*e
     * When set to true, the assignability algorithm will consider the type to
     * be serialized using XCDR encoding version 2.
     */
    RTIBool useXcdr2;
};
...
#define RTI_CDR_TYPE_OBJECT_ASSIGNABILITY_PROPERTY_DEFAULT {                  \
    RTI_FALSE /* ignoreMemberNames */,                                        \
    RTI_FALSE /* ignoreEnumLiteralNames */,                                   \
    RTI_FALSE /* ignoreSequenceBounds */,                                     \
    RTI_FALSE /* ignoreStringBounds */,                                       \
    RTI_FALSE /* preventTypeWidening */,                                      \
    RTI_FALSE /* useXcdr2 */                                                  \
}
