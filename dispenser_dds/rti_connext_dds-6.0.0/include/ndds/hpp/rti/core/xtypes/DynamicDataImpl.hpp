 ...
 * 
 * @subsection DDSDynamicDataModule_member_name_hierarchy Hierarchical Member Names
 * 
 * It is possible to refer to a nested member in a type without 
 * first having to use the loan_value() API. You can do this by
 * using a hierarchical name. A hierarchical member name is a
 * concatenation of member names separated by the '.' character.
 * The hierarchical name describes the complete path from a
 * top-level type to the nested member. For example, in the
 * following type:
 * 
 * \code
 * struct MyNestedType {
 ...
 * \endcode
 * 
 * any DynamicData API that receives a member name will accept
 * "theNestedType.theChar" to refer to the char member in
 * MyNestedType:
 * 
 * \code
 ...
 * \endcode
 * 
 * In order to access the value of \c theChar without using a 
 * hierarchical name, you would have to first bind to theNestedType 
 ...
 * \endcode
 *
 * As you can see, using a hierarchical member name removes the 
 * need to call the loan_value() API, and allows for access to
 * nested members at any depth directly from the top-level type.
 *
 * The member name can also contain indexes to address members 
 * in arrays and sequences. For example, to set the third member in 
 * the array \c theOctetArray, you can pass in 
 * "theNestedType.theOctetArray[2]" as the member name. The
 * index values when used as part of the member name are
 * 0-based.
 *
 * For multi-dimensional arrays, the indexes for each dimension 
 * should be listed comma-separated in between brackets. For 
 * example, to address a member of \c theMultidimensionalArray, the
 * member name should be something like 
 * "theNestedType.theMultidimensionalArray[3,2,5]".
 *
 * In complex types with arrays and sequences that contain other 
 * arrays and sequences, the hierarchical name may include multiple
 * index values, one right after another. For example, in 
 * MyNestedType, \c myArrayOfSeq is an array of sequences. In order 
 * to set the third member of the sequence in the fourth member of 
 * the array, the member name would be 
 * "myNestedType.myArrayOfSeq[3][2]".
 *
 *
...
public:
    ...
    /**
     * @brief Clear the contents of a single data member of this
     *        object.
     *
     * @param name The member name, see \ref DDSDynamicDataModule_member_name_id.
     */
 ...
 * @param representation_id The data representation used to serialize the data
