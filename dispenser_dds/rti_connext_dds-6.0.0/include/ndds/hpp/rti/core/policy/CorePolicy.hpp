 ...
 * @dref_DataRepresentationQosGroupDocs 
 * @dref_DataWriterProtocolQosGroupDocs 
 * @dref_DataWriterResourceLimitsQosGroupDocs
 * @dref_DataWriterTransferModeQosGroupDocs
 * @dref_DataTagQosGroupDocs
...
typedef detail::Liveliness Liveliness;
...
OMG_DDS_POLICY_TRAITS(rti::core::policy::DataWriterTransferMode, 1039)
...
class OMG_DDS_API DataRepresentationImpl : public NativeValueType<DataRepresentationImpl> {
public:
    RTI_NATIVE_VALUE_TYPE_DEFINE_DEFAULT_MOVE_OPERATIONS(DataRepresentationImpl)

public:

    typedef NativeValueType<DataRepresentationImpl> Base;

    /**
     * @brief Extended CDR Representation encoding version 1
     * 
     * Returns the representation ID corresponding to CDR version 1
     */
    static dds::core::policy::DataRepresentationId xcdr()
    {
        return DDS_XCDR_DATA_REPRESENTATION;
    }

    /**
     * @brief XML Data Representation
     * 
     * Returns the representation ID corresponding to XML
     */
    static dds::core::policy::DataRepresentationId xml()
    {
        return DDS_XML_DATA_REPRESENTATION;
    }

    /**
     * @brief Extended CDR Representation encoding version 2
     * 
     * Returns the representation ID corresponding to CDR version 2
     */
    static dds::core::policy::DataRepresentationId xcdr2()
    {
        return DDS_XCDR2_DATA_REPRESENTATION;
    }

    /**
     * @brief Representation is automatically chosen based on the type
     * 
     * @details \dref_details_DDS_AUTO_DATA_REPRESENTATION
     */
    static dds::core::policy::DataRepresentationId auto_id()
    {
        return DDS_AUTO_DATA_REPRESENTATION;
    }
    ...
    DataRepresentationImpl()
    {
        ...
        data_vector.push_back(auto_id());
        value(data_vector.begin(), data_vector.end());
    }

    explicit DataRepresentationImpl(
            const dds::core::policy::DataRepresentationIdSeq& sequence)
    {
        value(sequence.begin(), sequence.end());
    }

    DataRepresentationImpl(
            const dds::core::policy::DataRepresentationId* value_begin,
            const dds::core::policy::DataRepresentationId* value_end);

    template <typename SHORT_ITER>
    DataRepresentationImpl& value(SHORT_ITER the_begin, SHORT_ITER the_end)
    {
        value_impl(
            the_begin,
            the_end,
            typename std::iterator_traits<SHORT_ITER>::iterator_category());

        return *this;
    }

    dds::core::policy::DataRepresentationIdSeq value() const
    {
        return dds::core::policy::DataRepresentationIdSeq(begin(), end());
    }

    dds::core::policy::DataRepresentationIdSeq& value(
            dds::core::policy::DataRepresentationIdSeq& dst) const;

    const dds::core::policy::DataRepresentationId* begin() const;
    const dds::core::policy::DataRepresentationId* end() const;

private:
    template <typename SHORT_ITER>
    void value_impl(
        SHORT_ITER the_begin, SHORT_ITER the_end, std::random_access_iterator_tag)
    {
        // Knowing the size of the range [begin, end) allows for an optimized
        // implementation.
        resize(the_end - the_begin);
        std::copy(
                the_begin,
                the_end,
                DDS_DataRepresentationIdSeq_get_contiguous_buffer(
                        &native().value));
    }

    template <typename SHORT_ITER>
    void value_impl(
        SHORT_ITER the_begin, SHORT_ITER the_end, std::input_iterator_tag)
    {
        // Generic, less-efficient implementation for iterators that don't
        // support calculating the length of a range
        int sequence_index = 0;
        for (SHORT_ITER it = the_begin; it != the_end; ++it) {
            resize(sequence_index + 1);
            *DDS_DataRepresentationIdSeq_get_reference(
                    &native().value,
                    sequence_index) = *it;
            sequence_index++;
        }
...
public:
    ...
    typedef dds::core::policy::Liveliness WrapperType;
    ...
    /**
     * @memberof dds::core::policy::Liveliness
     * @brief @extension The number of assertions to send during the lease 
     * duration
     * 
     * \dref_details_LivelinessQosPolicy_assertions_per_lease_duration
     */
...
public:
    ...
    TypeConsistencyEnforcementImpl(
        dds::core::policy::TypeConsistencyEnforcementKind the_kind,
        bool the_ignore_sequence_bounds,
        bool the_ignore_string_bounds,
        bool the_ignore_member_names,
        bool the_prevent_type_widening,
        bool the_force_type_validation,
        bool the_ignore_enum_literal_names);
...
public:
    ...
    TypeConsistencyEnforcementImpl& ignore_sequence_bounds(bool ignore);
    bool ignore_sequence_bounds() const;

    TypeConsistencyEnforcementImpl& ignore_string_bounds(bool ignore);
    bool ignore_string_bounds() const;

    TypeConsistencyEnforcementImpl& ignore_member_names(bool ignore);
    bool ignore_member_names() const;

    TypeConsistencyEnforcementImpl& prevent_type_widening(bool prevent);
    bool prevent_type_widening() const;

    TypeConsistencyEnforcementImpl& force_type_validation(bool force);
    bool force_type_validation() const;

    TypeConsistencyEnforcementImpl& ignore_enum_literal_names(bool ignore);
    bool ignore_enum_literal_names() const;
};

// ----------------------------------------------------------------------------

class OMG_DDS_API DataTagImpl : public NativeValueType<DataTagImpl> {
public:
    /**
     * @brief The type of the elements that DataTag contains
     *
     * Key/value string pairs.
     */
    typedef std::pair<std::string, std::string> Entry;

    RTI_NATIVE_VALUE_TYPE_DEFINE_DEFAULT_MOVE_OPERATIONS(DataTagImpl)

    /**
     * @brief Creates an empty DataTag container
     */
    DataTagImpl() { }

    /**
     * @brief Creates a DataTag container with the entries specified by
     * an iterator range.
     *
     * @see set(EntryIter, EntryIter)
     */
    template <typename EntryIter>
    DataTagImpl(EntryIter begin, EntryIter end)
    {
        set(begin, end);
    }
    ...
    /**
     * @brief @st_cpp11 Creates a DataTag container with entries from an
     *        initializer_list
     *
     * For example:
     * @code
     ...
     * that comes last.
     */
    DataTagImpl(std::initializer_list<Entry> entries)
    {
        set(entries);
    }
  #endif
    ...
    /**
     * @brief Returns true if a tag exists
     */
    ...
    /**
     * @brief Returns the value of a tag identified by a key if it
     *        exists.
     *
     * If the tag doesn't exist it throws 
     * dds::core::PreconditionNotMetError. 
     */
    std::string get(const std::string& key) const;

    /**
     * @brief Returns the value of a tag identified by a key or an 
     * empty optional_value if it doesn't exist. 
     */
    rti::core::optional_value<std::string> try_get(const std::string& key) const;

    /**
     * @brief Adds or assigns a tag from a pair of strings
     *
     * If the key doesn't exists it adds the new entry, otherwise it overrides
     * its value with the new one.
     *
     * @param tag \p tag.first is the key and \p 
     *          tag.second is the value
     */
    DataTagImpl& set(const Entry& tag);

    /**
     * @brief Adds a range of tags
     *
     * If a key is duplicated, only one entry will remain, with the value
     * that comes last.
     *
     * @tparam EntryIter A forward iterator whose value type is 
     * DataTag::Entry, such as the iterators of a \c 
     * std::map<std::string, std::string>. 
     * @param begin Beginning of a range of Entry
     * @param end End of that range
     */
    template <typename EntryIter>
    DataTagImpl& set(EntryIter begin, EntryIter end)
    {
        for (; begin != end; ++begin) {
            set(*begin);
        }
    ...
    /**
     * @brief Removes the tag identified by a key
     * @return true if the tag was removed or false if it didn't 
     *         exist.
     */
  ...
  #ifndef RTI_NO_CXX11_HDR_INITIALIZER_LIST
    /**
     * @brief @st_cpp11 Adds tags from an initializer_list
     */
    void set(std::initializer_list<Entry> entries)
    {
        set(entries.begin(), entries.end());
    }
  #endif
};
...
public:
    ...
    /** \dref_DomainParticipantResourceLimitsQosPolicy_writer_data_tag_list_max_length
     */
    DomainParticipantResourceLimits& 
        writer_data_tag_list_max_length(int32_t max_length);
    ...
    /** \dref_DomainParticipantResourceLimitsQosPolicy_writer_data_tag_string_max_length
     */
    DomainParticipantResourceLimits& 
        writer_data_tag_string_max_length(int32_t max_length);
    ...
    /** \dref_DomainParticipantResourceLimitsQosPolicy_reader_data_tag_list_max_length
     */
    DomainParticipantResourceLimits& 
        reader_data_tag_list_max_length(int32_t max_length);
    ...
    /** \dref_DomainParticipantResourceLimitsQosPolicy_reader_data_tag_string_max_length
     */
    DomainParticipantResourceLimits& 
        reader_data_tag_string_max_length(int32_t max_length);
    ...
    /** \dref_DomainParticipantResourceLimitsQosPolicy_shmem_ref_transfer_mode_max_segments
     */
    DomainParticipantResourceLimits& 
...
public:
    ...
    /** \dref_DataWriterResourceLimitsQosPolicy_writer_loaned_sample_allocation */
    const AllocationSettings& writer_loaned_sample_allocation() const;

    /**
     * @brief Getter by non-const reference (see getter by const 
     *        reference with the same name)
     */
    AllocationSettings& writer_loaned_sample_allocation();

    /** \dref_DataWriterResourceLimitsQosPolicy_initialize_writer_loaned_sample */
...
/**
 * @RTI_class_definition class DataWriterTransferMode
 * @ingroup DDSDataWriterTransferModeQosModule
 * @headerfile rti/core/policy/CorePolicy.hpp
 *
 * @brief @extension Configures the transfer mode of a dds::pub::DataWriter
 *
 * @details \dref_details_DataWriterTransferModeQosPolicy
 */
class OMG_DDS_API DataWriterTransferMode : 
    public NativeValueType<DataWriterTransferMode> {
public:
    RTI_NATIVE_VALUE_TYPE_DEFINE_DEFAULT_MOVE_OPERATIONS(DataWriterTransferMode)

private:
    typedef NativeValueType<DataWriterTransferMode> Base;

public:
    /**
     * @brief Creates a DataWriterTransferMode qos policy with 
     *        default values
     */
    DataWriterTransferMode() {}

    /**
     * @brief Creates a policy with ShmemRefSettings
     */
    static DataWriterTransferMode ShmemRefSettings(
            bool enable_data_consistency_check)
    {
        return DataWriterTransferMode().shmem_ref_settings(
                rti::core::DataWriterShmemRefTransferModeSettings(
                        enable_data_consistency_check));
    }

    /** \dref_DataWriterTransferModeQosPolicy_shmem_ref_settings */
    DataWriterTransferMode& shmem_ref_settings(
        const rti::core::DataWriterShmemRefTransferModeSettings& settings);

    /**
     * @brief Getter by const reference (see setter)
     */
    const rti::core::DataWriterShmemRefTransferModeSettings&
        shmem_ref_settings() const;

    /**
     * @brief Getter by reference (see setter)
     */
...
public:
    ...
    /** \dref_DataReaderResourceLimitsQosPolicy_shmem_ref_transfer_mode_attached_segment_allocation */
...
public:
    ...
    /**
     * \dref_DiscoveryConfigQosPolicy_secure_volatile_writer_publish_mode
     */
    ...
    /**
     * \dref_DiscoveryConfigQosPolicy_endpoint_type_object_lb_serialization_threshold
     */
