...
RTI_POLICY_DEFINE_ADAPTER(DataRepresentation)
...
RTI_POLICY_DEFINE_SIMPLE_ADAPTER(OwnershipStrength, DDS_OWNERSHIP_STRENGTH_QOS_POLICY_DEFAULT)
...
RTI_POLICY_DEFINE_SIMPLE_ADAPTER(DataWriterTransferMode, DDS_DataWriterTransferModeQosPolicy_INITIALIZER)
...
public:
    ...
    static void copy(native_type& destination, const native_type& source)
    {
        if (DDS_PropertyQosPolicy_copy(&destination, &source) == NULL) {
            throw std::bad_alloc();
        }
    }
...
class DataTagAdapter {
public:
    typedef DDS_DataTagQosPolicy native_type;

    static void initialize(native_type& native_value)
    {
        DDS_DataTags_initialize(&native_value);
    }

    static void finalize(native_type& native_value)
    {
        DDS_DataTags_finalize(&native_value);
    }

    static void copy(native_type& destination, const native_type& source)
    {
        if (DDS_DataTags_copy(&destination, &source) == NULL) {
            throw std::bad_alloc();
        }
    }
    ...
    static bool equals(const native_type& first, const native_type& second)
    {
        return (DDS_DataTagQosPolicy_equals(&first, &second)
                == DDS_BOOLEAN_TRUE);
    }
};
...
RTI_POLICY_DECLARE_ADAPTER_TRAITS(DataRepresentationImpl, DataRepresentationAdapter)
...
RTI_POLICY_DECLARE_ADAPTER_TRAITS(DataTagImpl, DataTagAdapter)
...
RTI_POLICY_DECLARE_ADAPTER_TRAITS(DataWriterTransferMode, DataWriterTransferModeAdapter)
