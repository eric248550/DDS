...
// its size is not the same as what goes on the wire (4 bytes)
template <>
struct sequence_serialization_helper<wchar_t> {

    // When sizeof(wchar_t) == RTI_CDR_WCHAR_SIZE we don't need any
    // special conversion
    template <typename C>
    static
    typename rtiboost::enable_if_c<
        sizeof(typename C::value_type) == RTI_CDR_WCHAR_SIZE,
        bool>::type
    serialize(
            struct RTICdrStream *me,
            const C& v,
    ...
    // When sizeof(wchar_t) != RTI_CDR_WCHAR_SIZE we need to cast and serialize
    // element by element
    template <typename C>
    static
    typename rtiboost::enable_if_c<
    ...
    // When sizeof(wchar_t) != RTI_CDR_WCHAR_SIZE we need to deserialize a
    // 4 bytes and then cast it to wchar_t, element by element
    template <typename C>
    static
    typename rtiboost::enable_if_c<
        sizeof(typename C::value_type) != RTI_CDR_WCHAR_SIZE,
