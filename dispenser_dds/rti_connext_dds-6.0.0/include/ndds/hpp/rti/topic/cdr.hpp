...
typename rtiboost::enable_if_c<
    ...
    initialize_sample_impl(TopicType& sample)
{
    dds::topic::topic_type_support<TopicType>::allocate_sample(sample, 0, 0);
}
 ...
 * @param representation The representation to serialize the data into
 */
template <typename TopicType>
 ...
 * @brief Serializes a sample into a buffer of octets in CDR format
 *
 * This overload uses dds::core::policy::DataRepresentationId::auto_id() as the
 * data representation.
 */
template <typename TopicType>
std::vector<char>& to_cdr_buffer(
