...
#ifndef RTI_REQUEST_DETAIL_GENERICRCEIVER_HPP_
#define RTI_REQUEST_DETAIL_GENERICRCEIVER_HPP_
...
public:
    ...
    // For two-phase initialization only. initialize() must be called after this.
    GenericReceiver() :
            reader_(dds::core::null),
            any_sample_cond_(dds::core::null),
            not_read_sample_cond_(dds::core::null),
            max_samples_per_read_(INT_MAX)
    {
    }
    ...
    GenericReceiver(
            ...
            : reader_(dds::core::null),
              ...
              max_samples_per_read_(INT_MAX)
    {
        initialize(params, topic_desc, role_name, listener, mask);
    }
        ...
        const dds::core::status::StatusMask& mask)
    {
        ...
        if (subscriber == dds::core::null) {
            // Use implicit subscriber
            subscriber = rti::sub::implicit_subscriber(params.participant());
        }
        ...
        try {
            ...
            reader_ = DataReader<T>(subscriber, cft, qos, listener, mask);
        } catch (const dds::core::InvalidDowncastError&) {
            Topic<T> topic = dds::core::polymorphic_cast<Topic<T> >(topic_desc);
            reader_ = DataReader<T>(subscriber, topic, qos, listener, mask);
        }
        ...
        max_samples_per_read_ = qos.policy<rti::core::policy::DataReaderResourceLimits>()
...
#endif // RTI_REQUEST_DETAIL_GENERICRCEIVER_HPP_
