...
template <typename Tester, typename TestFunc>
...
//
// Note: std::men_fun has been deprecated in C++11 and removed in C++17. It's
// been replaced by std::mem_fn.
...
// The RTI_TEST_FUNCTION_ADD_ADDITIONAL macro was created to support splitting
// tests as basic and additional. Tests added with this macro will belong to
// the additional test set
#define RTI_TEST_FUNCTION_ADD_ADDITIONAL(CLASS_NAME, TEST_FUNCTION)           \
    struct runner_ ## TEST_FUNCTION {                                         \
        static int run(const struct RTITestSetting* setting)                  \
        {                                                                     \
            return run_test_function(                                         \
                ...
                std::mem_fun(&CLASS_NAME::TEST_FUNCTION));                    \
                                                                              ...
                                                                              \
    add_test_function_additional((RTITestFunction) runner_ ## TEST_FUNCTION::run)
...
public:
    ...
    int get_test_count_additional() const
    {
        return static_cast<int>(_test_functions_additional.size());
    }

    const RTITestFunction* get_test_functions() const
    {
        return (_test_functions.empty() ? NULL : &_test_functions[0]);
    }

    const RTITestFunction* get_test_functions_additional() const
    {
        return (_test_functions_additional.empty() ?
                NULL :
                &_test_functions_additional[0]);
    }
    ...
    void add_test_function_additional(RTITestFunction fnc)
    {
        _test_functions_additional.push_back(fnc);
    }
    ...
    int ignore_test(const std::string& reason = "") const
    {
        ...
        if (reason != "") {
            RTITestLog_freeForm(RTI_LOG_BIT_WARN)("%s", (": " + reason).c_str());
        }
    ...
    virtual RTIBool run_tests(const RTITestSetting * settings)
    {
        ...
        return RTITestSetting_runTestsExt(settings,
                                       _name.c_str(),
                                       get_test_functions(),
                                       get_test_count(),
                                       get_test_functions_additional(),
                                       get_test_count_additional(),
                                       begin, end);
    }
...
private:
    ...
    void merge_tester(Tester& src_tester)
    {
        ...
        const RTITestFunction * src_functions_additional = src_tester.get_test_functions_additional();
        const int test_count_additional = src_tester.get_test_count_additional();

        for (int i = 0; i < test_count; i++) {
            add_test_function(src_functions[i]);
        }
        for (int i = 0; i < test_count_additional; i++) {
            add_test_function_additional(src_functions_additional[i]);
        }
    }
    ...
    std::vector<RTITestFunction> _test_functions_additional;
