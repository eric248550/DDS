...
public:
    ...
    {
        // The consumer/producer listeners lock a weak_ref to obtain the 
        // QueueReplier. We can only do that if the_listener (user listener) is
        // not null. Otherwise we run into the race condition described in 
        // QUEUEING-688: when the main application ends after the weak_ref has 
        // been locked but before the listener callback has finished, the QueueReplier
        // is destroyed inside the listener callback, causing a EA inversion error
        // 
        // The solution is to avoid locking the weak_ref if there is not a user 
        // listener attached to the Replier/Requester--the presence of a user
        // listener retains the QueueReplier forcing the application to detach it,
        // which makes the race condition impossible. To do that we only set
        // the internal listeners if there is also a user listener. Note that
        // consumer_ and producer_ have other internal DataWriter listeners 
        // of their own.
