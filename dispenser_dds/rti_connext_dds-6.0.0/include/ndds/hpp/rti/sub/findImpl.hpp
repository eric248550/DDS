 ...
 *  
 * @tparam Reader The type of the reader. It can be dds::sub::AnyDataReader, or
 * an instantiation of dds::sub::DataReader<T> (if T is not the correct
 * type, this function throws dds::core::InvalidDowncastError)
 ...
 * to copying the found DataReader into
 ...
 * @param begin A forward iterator to the position in the destination container 
 * to being copying the found DataReaders into. This container should be a container
    ...
    uint32_t max_size)
{
    ...
    DDS_ReturnCode_t retcode = DDS_Subscriber_get_datareaders(
        ...
        &native_readers,
        static_cast<unsigned int>(data_state.sample_state().to_ulong()),
        static_cast<unsigned int>(data_state.view_state().to_ulong()),
        static_cast<unsigned int>(data_state.instance_state().to_ulong()));
 ...
 * this function, whose value_type must be dds::sub::Subscriber.
 *
 * @param participant The dds::domain::DomainParticipant the Publishers 
 * belong to
 * @param begin A forward iterator to the position in the destination container 
 * where to begin copying the found Subscribers into
 ...
 * passed into this function, whose value_type must be dds::sub::Subscriber.
 *
 ...
 * 
 * @tparam AnyDataReaderBackInsertIterator Type of the back-inserting iterator
 * passed into this function, whose value_type must be dds::sub::AnyDataReader.
 *
 * @param participant The dds::domain::DomainParticipant the Subscribers 
 * belong to
 ...
 *
 * @see \ref DDSReaderExampleModule_lookup
 */
template <typename AnyDataReaderBackInsertIterator>
 ...
 * @param begin A forward iterator to the position in the destination container 
 * where to begin copying the found readers into.
 ...
 * @return The DataReader with the given topic name, or dds::core::null if it doesn't exist
 *
 * @see \ref DDSReaderExampleModule_lookup
 */
 ...
 * @return The DataReader with the given name, or dds::core::null if it doesn't exist
 *
 * @see \ref DDSReaderExampleModule_lookup
 */
template<typename Reader>
 ...
 * 
 * @tparam Reader The type of the reader returned, for example,
 * dds::sub::DataReader<Foo>, or dds::sub::AnyDataReader
 *
 ...
 * find 
 * 
 * @return The found DataReader, or dds::core::null if it doesn't exist
 */
