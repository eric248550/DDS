 ...
 * @brief The definition of the dds::core::safe_enum rti::sub::TopicQuerySelectionKind
 */
struct TopicQuerySelectionKind_def {
    ...
    enum type {
        /**
         * \dref_TopicQuerySelectionKind_HISTORY_SNAPSHOT
         */
        HISTORY_SNAPSHOT = DDS_TOPIC_QUERY_SELECTION_KIND_HISTORY_SNAPSHOT,
        /**
         * \dref_TopicQuerySelectionKind_CONTINUOUS
         */
        CONTINUOUS = DDS_TOPIC_QUERY_SELECTION_KIND_CONTINUOUS,
    };
};
...
/**
 * @ingroup DDSTopicQueryModule
 * @brief @ref SafeEnumModule of TopicQuerySelectionKind_def
 * @see TopicQuerySelectionKind_def for the enumerated values
 */
...
public:
    ...
    /**
     * @brief Alias for TopicQuerySelectionKind
     */
     ...
     *
     * @param the_filter Defines the data query
     * 
     * @note The filter expression can start with the special condition 
     * "@instance_state = ALIVE AND" followed by the rest of the expression.
     * This restricts the selection to samples of alive instances.
     *
     * @see TopicQuery::SelectAll()
     * @see TopicQuery::UseReaderContentFilter()
     */
    ...
    /**
     * @brief Creates a TopicQuerySelection with a selection kind
     *
     * @param the_filter Defines the data query
     * @param the_kind Indicates the kind of selection
     */
    TopicQuerySelection(const dds::topic::Filter& the_filter, Kind the_kind)
        : filter_(the_filter), kind_(the_kind)
    {
    }
    ...
    {
        return filter() == other.filter() && kind() == other.kind();
    }
    ...
    friend void swap(TopicQuerySelection& a, TopicQuerySelection& b) OMG_NOEXCEPT
    {
        swap(a.filter_, b.filter_);
        swap(a.kind_, b.kind_);
    }
 ...
 * 
 * @note A TopicQuery provides all the functions of a @st_ref_type, including 
 * close() and retain().
 *
