...
/**
 * @brief Base class for all reference types
 *
 * The behavior and members of this class are described here: @st_ref_type
 *
 * @see @st_ref_type
 */
...
public:
    operator DELEGATE_REF_T() const OMG_NOEXCEPT
    {
        return impl_;
    }
     ...
     */
    template <typename R>
    bool operator==(const R& ref) const OMG_NOEXCEPT
    {
     ...
     */
    bool is_nil() const OMG_NOEXCEPT
    {
     ...
     */
    bool operator==(const null_type) const OMG_NOEXCEPT
    {
        return this->is_nil();
    }
...
public:
    ...
    /*
     * Provides access to extension methods (just like operator->)
     *
     *  <code>
     *      my_dds_entity.standard_method();
     *      my_dds_entity.extensions().vendor_specific_extension();
     *  </code>
     *
     * @return a reference to delegate.
     */
    const DELEGATE& extensions() const
    {
        if (!impl_) {
            throw dds::core::NullReferenceError(
                    "cannot access extensions(): null reference");
        }
        return *impl_;
    }
...
template <class D>
bool operator ==(dds::core::null_type, const dds::core::Reference<D> & ref) OMG_NOEXCEPT
{
    return ref.is_nil();
}
