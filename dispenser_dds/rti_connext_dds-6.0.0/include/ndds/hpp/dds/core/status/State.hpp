...
public:
    ...
    /**
     * @brief @extension A sample has been removed from a
     * dds::pub::DataWriter
     *
     * @details \dref_details_StatusKind_DATA_WRITER_SAMPLE_REMOVED_STATUS
     */
    static const StatusMask sample_removed()
    {
        return StatusMask(DDS_DATA_WRITER_SAMPLE_REMOVED_STATUS);
    }

    /**
     * @brief @extension A locator is unreachable from a
     * dds::pub::DataWriter
     *
     */
    static const StatusMask destination_unreachable()
    {
        return StatusMask(DDS_DATA_WRITER_DESTINATION_UNREACHABLE_STATUS);
    }
};
