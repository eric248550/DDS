...
public:
   ...
   * 
   * @see \ref NDDSFilterExampleModule_query_conditions for an example on how
  ...
  TQueryCondition(
      ...
      : dds::sub::cond::TReadCondition<DELEGATE>(
          new DELEGATE(query, status))
  {
      this->delegate()->remember_reference(this->delegate(), functor);
  }

  // Spec issue CPPPSM-18: need this constructor to make
  // polymorphic_cast work and creation from native entity
  TQueryCondition(DELEGATE_REF_T& delegate_ref)
      : TReadCondition<DELEGATE>(delegate_ref)
  {
      if (this->delegate()) {
          this->delegate()->remember_reference(this->delegate());
      }
